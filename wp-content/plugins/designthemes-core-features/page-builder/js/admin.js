jQuery(document).ready(function($){
	...
	$( 'body' ).delegate( '.dt_settings_arrow_column', 'click', function(){
		var dt_action = 'dt_show_columoptions_panel', this_column = $(this),
			 dt_column_type = dt_column_id = dt_column_css = dt_column_animation_type = dt_column_animation_delay = '';
			 
		$(this).closest('.dt_m_column').addClass('dt_active_column');
		var dt_data = $(this).closest('.dt_m_column').find('.dt_columndata_settings').html();
		if(dt_data != '') {
			dt_column_type = $(dt_data).attr('data-column_type');
			dt_column_id = $(dt_data).attr('data-column_id');
		...
		$.ajax({
			...
			data:
			{
				action : dt_action,
				dt_load_nonce : dtthemes_options.dt_load_nonce,
				dt_column_type : dt_column_type,
				dt_column_id : dt_column_id,
	...
	$( 'body' ).delegate( 'form#dt_column_settings span#save_columnoptions', 'click', function(){
		...
		var column_type = column_id = column_css = column_animation_type = column_animation_delay = '', $save_message = $("#dtthemes_ajax_save");
		...
		if($('#dt_column_settings #dt_column_type').val() != '' && $('#dt_column_settings #dt_column_type').val() != undefined)
			column_type = ' data-column_type="' + $('#dt_column_settings #dt_column_type').val() + '"';
		if($('#dt_column_settings #dt_column_id').val() != '' && $('#dt_column_settings #dt_column_id').val() != undefined)
			column_id = ' data-column_id="' + $('#dt_column_settings #dt_column_id').val() + '"';
		...
		$('.dt_active_column .dt_columndata_settings').append( '<div id="column_data" ' + column_type + column_id + column_css + column_animation_type + column_animation_delay + '></div>' );
			...
			if($this_module.hasClass('dt_fullwidth_section')) {	
				...
				$this_module.find('.dt_fullwidth_section_container > .dt_module').each( function() {
					...
					if($current_module.hasClass('dt_m_column')) {
						...
						if($current_module.find('#column_data').attr('data-dt_column_type') != undefined && $current_module.find('#column_data').attr('data-dt_column_type') != '')			
							shortcode_output += ' type="' + $current_module.find('#column_data').attr('data-dt_column_type') + '"';

						if($current_module.find('#column_data').attr('data-column_id') != undefined && $current_module.find('#column_data').attr('data-column_id') != '')			
							shortcode_output += ' id="' + $current_module.find('#column_data').attr('data-column_id') + '"';
				...
				if($this_module.find('#column_data').attr('data-column_type') != undefined && $this_module.find('#column_data').attr('data-column_type') != '')			
					shortcode_output += ' type="' + $this_module.find('#column_data').attr('data-column_type') + '"';
				if($this_module.find('#column_data').attr('data-column_id') != undefined && $this_module.find('#column_data').attr('data-column_id') != '')			
					shortcode_output += ' id="' + $this_module.find('#column_data').attr('data-column_id') + '"';
