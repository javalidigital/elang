...
function dt_theme_enqueue_scripts() {
	...
	if( is_page_template('tpl-onepage.php') )
		wp_enqueue_script('jq.scrollto', IAMD_FW_URL.'js/public/jquery.scrollTo.js',array(),false,true);
	else
	...
	wp_enqueue_script('jq.meanmenu', IAMD_FW_URL.'js/public/jquery.meanmenu.js', array(), false, true);
	...
	wp_enqueue_script('jq.parallax', IAMD_FW_URL.'js/public/jquery.parallax-1.1.3.js', array(), false, true);
	wp_enqueue_script('jq.animatenum', IAMD_FW_URL.'js/public/jquery.animateNumber.min.js', array(), false, true);
	
	if( is_page_template('tpl-onepage.php') )
		wp_enqueue_script('jq.onepage.nav', IAMD_FW_URL.'js/public/jquery.nav.js', array(), false, true);
	
	wp_enqueue_script('jq.custom', IAMD_FW_URL.'js/public/custom.js', array(), false, true);
}
...
function dt_theme_appearance_load_fonts() {
	...
	if (!empty($custom_fonts)) :
		...
		$font .= "Open+Sans:300,400,600,700|Droid+Serif:400,400italic,700,700italic|Pacifico|Patrick+Hand|Crete+Round:400";
...
function dt_theme_appearance_css() {
	...
	if (empty($disable_styling)) :
		...
		if(!empty($styling_header_bg_color) and $styling_header_bg_color != "#")
			$output .= "#header-wrapper { background-color: $styling_header_bg_color; }" ;
		...
		if(!empty($styling_footer_bg_color) and $styling_footer_bg_color != "#")
			$output .= "#footer { background-color: $styling_footer_bg_color; }" ;
		...
		if(!empty($styling_sub_footer_bg_color) and $styling_sub_footer_bg_color != "#")
			$output .= ".footer-info { background-color: $styling_sub_footer_bg_color; }" ;
			
		if(!empty($styling_sub_footer_text_color) and $styling_sub_footer_text_color != "#")
			$output .= "#footer p.copyright, #footer p.copyright a, #footer #footer-menu li a  { color: $styling_sub_footer_text_color; }" ;
	...
	function simple_breadcrumb_case($der_post) {
		...
		if (is_single()) {
			...
			if (is_attachment()) {
				...
				if(!is_wp_error(get_category_parents($ID, TRUE, $markup, FALSE)))
		...
		if (is_post_type_archive('course')) {
			return __("Courses", 'iamd_text_domain');
		}

		if (is_post_type_archive('lesson')) {
			return __("Lessons", 'iamd_text_domain');
		}
...
function dt_theme_color_picker(){
	...
	$str .= '	</div>';
	$str .= '</div><!-- **GURU Style Picker Wrapper - End** -->';
